...
"""
    四通道（都是基于transformer编码的SMILES特征）；三组学；后期特征融合
"""



class Conv_TransMCA_GEP_CNV_MUT_V2(nn.Module):
    ...
    """
        ...
        super(Conv_TransMCA_GEP_CNV_MUT_V2, self).__init__(*args, **kwargs)
    ...
    def forward(self, smiles, gep, cnv, mut):
        ...
        encoded_smiles = [trans_smiles] + [
            self.convolutional_layers[ind]
            (torch.unsqueeze(trans_smiles, 1)).permute(0, 2, 1)
